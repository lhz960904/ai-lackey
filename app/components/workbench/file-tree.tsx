...
function Folder({ folder: { depth, name }, collapsed, selected = false, onClick }: FolderProps) {
  return (
    <NodeButton
      ...
      depth={depth}
      icon={collapsed ? <FolderIcon size={14} /> : <FolderOpen size={14} />}
      onClick={onClick}
    >
      {name}
    </NodeButton>
...
function File({ file: { depth, name }, onClick, selected, unsavedChanges = false }: FileProps) {
  return (
    <NodeButton
      className={cn('group', {
        'bg-transparent hover:bg-gray-100': !selected,
...
function NodeButton({ depth, onClick, className, icon, children }: ButtonProps) {
  return (
    <button
      className={cn(
        'flex items-center gap-1.5 w-full pr-2 border-2 border-transparent text-faded py-0.5 cursor-pointer',
        className,
      )}
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
      onClick={() => onClick?.()}
    >
      <div className={cn('scale-120 shrink-0')}>
        {icon}
      </div>
