...
import { DEFAULT_FILE_FILTERING_OPTIONS, FileFilteringOptions } from "../config/config";
...
async function readFullStructure(
  ...
  // To avoid processing same path if symlinks create loops
      ...
      if (entry.isFile()) {
        ...
        if (options.fileService) {
          const shouldIgnore =
            ...
            (options.fileFilteringOptions.respectLackeyIgnore &&
...
export async function getFolderStructure(
  ...
  try {
    ...
    console.log('structureRoot', structureRoot);
